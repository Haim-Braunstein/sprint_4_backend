...
export function setupSocketAPI(server) {
    ...
    gIo.on('connection', socket => {
        ...
        socket.on('station-updated', (updatedStation) => {
            console.log("🚀 ~ file: socket.service.js:20 ~ socket.on ~ updatedStation:", updatedStation)
            logger.info(`Station updated [id: ${updatedStation._id}], broadcasting changes.`)
               socket.to(socket.myTopic).emit('station-updated', updatedStation)
            })
